...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul  5 09:48:34 2020
...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul  5 09:25:18 2020
...
def prepare_dataset_separate(data_path,out_dir ,protein_featurizer, pocket_featurizer=None,
                    ids=None, hdf_mode='w',
    ...
    file_with_prefixes_data_name='/media/user1/New Volume/py/fcnlabel/PDB_IDall.pkl'


    with open(file_with_prefixes_data_name, 'rb') as file_with_prefixes_data:
        P_name = pickle.load(file_with_prefixes_data)

    prot_n = P_name['PDB ID']
    multiple_pockets = {}

    for structure_id in progress_bar(prot_n):
        group_id = structure_id
        multiple_pockets[group_id] = multiple_pockets.get(group_id, 1) + 1
        out_prefix = os.path.join(out_dir, group_id)

        if os.path.isfile(out_prefix):
#            print('Skipping. Files "%s" already exist.' % (out_prefix))
            continue
        value=prot_n[structure_id]
        pocket_path_o, protein_path_o = get_paths(data_path, value[0])
        pocket_o = next(pybel.readfile('mol2', pocket_path_o))
        protein_o = next(pybel.readfile('mol2', protein_path_o))

        pocket_coords_o, pocket_features_o = featurize_pocket(pocket_o)
        prot_coords_o, prot_features_o = protein_featurizer.get_features(protein_o)

        centroid_o = prot_coords_o.mean(axis=0)
        pocket_coords_o -= centroid_o
        prot_coords_o -= centroid_o

        with h5py.File(out_prefix, mode=hdf_mode) as f:
            for jj in range(1,len(value)):
                name=value[jj]
                pocket_path, protein_path = get_paths(data_path, value[jj])
                ...
                if not np.allclose(centroid_o,centroid , atol=0.5):
                ...
                multiple_pockets[group_id] = multiple_pockets.get(group_id, 1) + 1
                pocket_coords_o=np.concatenate((pocket_coords_o,pocket_coords))
                pocket_features_o=np.concatenate((pocket_features_o,pocket_features))
            # first pocket for the structure - create all datasets
            group = f.create_group(group_id)
            for key, data in (('coords', prot_coords_o),
                              ('features', prot_features_o),
                              ('pocket_coords', pocket_coords_o),
                              ('pocket_features', pocket_features_o),
                              ('centroid', centroid_o)):
...
out_dir = 'kalasanty_scpdb'
if not os.path.exists(out_dir):
    os.makedirs(out_dir)
data_path = "/media/user1/New Volume/py/fcnlabel/scPDB2017"
protein_featurizer = Featurizer(save_molecule_codes=False)

prepare_dataset_separate(data_path,out_dir, protein_featurizer, pocket_featurizer=None,
                    ids=None,  hdf_mode='w',
                    progress_bar=None, db_format='scpdb', verbose=False)
...
# data_handle = {}
# pdbids=os.listdir('/media/user1/New Volume/Project/python_code/kalasanty-master2/kalasanty_scpdb')
# hdf_path=out_dir
# pdbids=pdbids
#for pid in pdbids:
#    out_prefix = os.path.join(hdf_path, pid)
#
#    with h5py.File(out_prefix, mode='r') as f:
#        data_handle[pid] = {}
#        for key in keys:
#            data_handle[pid][key] = f[pid][key][:]
# def get_box_size(scale, max_dist):
#     return int(np.ceil(2 * max_dist * scale + 1))

# class DataWrapperseparate():
...
# #        pdbid = self.pdbids[0]
# #        out_prefix = os.path.join(self.hdf_path, pdbid)
# #        self.data_handle = {}
# #        with h5py.File(out_prefix, mode='r') as f:
# #
# #            self.data_handle[pdbid] = {}
# #            for key in keys:
# #                self.data_handle[pdbid][key] = f[pid][key][:]
#         self.x_channels = 18
#         self.y_channels = 1
...
#         if self.pdbids is None:
#             self.pdbids = os.listdir(self.hdf_path)
#         self.closed = False
...
# data=DataWrapper(hdf_path, pdbids=pdbids)
